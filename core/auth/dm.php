...
$users_query = "SELECT user_id, name, email FROM users WHERE user_id != ? ORDER BY name";
    ...
    if ($_POST['action'] === 'send_message') {
        ...
        if (!empty($message_text) && $receiver_id > 0) {
            ...
            if ($stmt->execute()) {
                ...
                // Store message content in a message_content field (we'll need to alter the table)
                // For now, we'll create a messages table to properly store the content
...
// Get conversation if a user is selected
                ...
                <div class="users-list">
                    <?php if (!empty($available_users)): ?>
                        <?php foreach ($available_users as $user): ?>
                            <a href="dm.php?user_id=<?php echo $user['user_id']; ?>" 
                               class="user-item <?php echo ($conversation_with_id === $user['user_id']) ? 'active' : ''; ?>">
